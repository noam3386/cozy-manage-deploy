...
import { Building2, Loader2, Mail, Lock, User, Home, Eye, EyeOff, Phone } from 'lucide-react';
...
const Auth = () => {
  ...
  const [role] = useState<'owner'>('owner');
  ...
  const redirectBasedOnRole = async (userId: string) => {
    ...
    const roles = error ? [] : (data ?? []).map((r) => r.role as 'admin' | 'owner' | 'manager');
    const target = roles.includes('admin') || roles.includes('manager') ? '/manager' : '/owner';

    setAppRole(target === '/manager' ? 'manager' : 'owner');
